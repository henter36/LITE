...
router.get("/audit-logs", async (req, res) => {
  ...
  const items = whereClause
    ? await db.select().from(auditLogsTable).where(whereClause).orderBy(auditLogsTable.createdAt).limit(limit).offset(offset)
    : await db.select().from(auditLogsTable).orderBy(auditLogsTable.createdAt).limit(limit).offset(offset);
