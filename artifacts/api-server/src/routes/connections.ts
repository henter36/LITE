...
// Safety guard: reject any real publishing/budget change attempts
function rejectRealOps(req: any, res: any) {
...
function serialize(c: typeof platformConnectionsTable.$inferSelect) {
  return {
    ...
    status: c.status,
    mockSpend: c.mockSpend,
    mockImpressions: c.mockImpressions,
    mockClicks: c.mockClicks,
...
router.post("/connections", async (req, res) => {
  ...
  // Mock: generate fake account ID and mock metrics
  ...
  const [conn] = await db.insert(platformConnectionsTable).values({
    workspaceId, platform, accountName,
    accountId: mockAccountId,
    status: "connected",
    mockSpend, mockImpressions, mockClicks,
    lastSyncAt: new Date(),
  }).returning();
  await db.insert(auditLogsTable).values({ workspaceId, action: "mock_connection_created", entityType: "connection", entityId: conn.id, actor: "user", details: `Mock ${platform} account "${accountName}" connected` });
...
router.post("/connections/:id/sync", async (req, res) => {
  ...
  await db.insert(auditLogsTable).values({ workspaceId: conn.workspaceId, action: "mock_sync_executed", entityType: "connection", entityId: id, actor: "system", details: `Mock sync for ${conn.platform} account "${conn.accountName}"` });
